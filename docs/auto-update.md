...
- **Windows** — the `resources/package-type` marker the packager stamps for MSI (see below), the `PORTABLE_EXECUTABLE_FILE` environment variable (portable), a `WindowsApps` install path (AppX/MSIX), or the `Uninstall <AppName>.exe` the NSIS installer writes into the install directory.

An install directory that can be read but contains no NSIS uninstaller is treated as MSI — the only signal left by an MSI built before the marker existed. An install directory that cannot be read at all is treated as NSIS instead, so a transient failure (restrictive ACLs, an unresolvable install root) cannot silently disable updates forever.
