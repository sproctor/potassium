...
| **S** | [Nucleus](https://github.com/NucleusFramework/Nucleus) | 88 | MIT (free) |
...
|------|------|---------|--------|--------|
| **Nucleus** (upstream of Potassium) | Gradle plugin + runtime modules | v2.1.10 | Active | [GitHub](https://github.com/NucleusFramework/Nucleus) |
...
??? info "Sources"
    - **Potassium**: macOS universal binary via [`build-macos-universal`](https://github.com/sproctor/potassium/tree/main/.github/actions/build-macos-universal) CI action using `lipo`
...
??? info "Sources"
    - **Potassium**: [`TargetFormat.kt`](https://github.com/sproctor/potassium/blob/main/plugin/src/main/kotlin/com/seanproctor/potassium/dsl/TargetFormat.kt) — 17 enum values (16 distributable + `JpackageImage` intermediate)
...
??? info "Sources"
    - **Potassium**: [`MacOSSigningSettings.kt`](https://github.com/sproctor/potassium/blob/main/plugin/src/main/kotlin/com/seanproctor/potassium/dsl/MacOSSigningSettings.kt), [`WindowsSigningSettings.kt`](https://github.com/sproctor/potassium/blob/main/plugin/src/main/kotlin/com/seanproctor/potassium/dsl/WindowsSigningSettings.kt) — Azure Artifact Signing via `azureTenantId`, `azureEndpoint`, `azureCertificateProfileName`
...
??? info "Sources"
    - **Potassium**: 5 composite actions in [`.github/actions/`](https://github.com/sproctor/potassium/tree/main/.github/actions) — `setup-potassium` (JBR or GraalVM Liberica NIK + tools), `setup-macos-signing` (keychain + P12), `build-macos-universal` (lipo merge + re-sign), `build-windows-appxbundle` (MakeAppx + SignTool), `publish-github-release` (merges per-arch update manifests, then `gh release create` with the installers + electron-builder's `latest-*.yml`). `setup-potassium` supports a `graalvm` option to install BellSoft Liberica NIK instead of JBR, enabling GraalVM Native Image builds in CI.
...
??? info "Sources"
    - **Potassium**: [`AbstractGenerateAotCacheTask.kt`](https://github.com/sproctor/potassium/blob/main/plugin/src/main/kotlin/com/seanproctor/potassium/tasks/AbstractGenerateAotCacheTask.kt) — Project Leyden via `-XX:AOTCacheOutput` (JDK 25+); [`ProguardSettings.kt`](https://github.com/sproctor/potassium/blob/main/plugin/src/main/kotlin/com/seanproctor/potassium/dsl/ProguardSettings.kt) — ProGuard 7.9.1 default; [`AbstractJLinkTask.kt`](https://github.com/sproctor/potassium/blob/main/plugin/src/main/kotlin/com/seanproctor/potassium/tasks/AbstractJLinkTask.kt) — jlink with strip-debug, compression; [`AbstractPatchCaCertificatesTask.kt`](https://github.com/sproctor/potassium/blob/main/plugin/src/main/kotlin/com/seanproctor/potassium/tasks/AbstractPatchCaCertificatesTask.kt) — copies JLink runtime, runs keytool to import PEM/DER certificates into `lib/security/cacerts`; `graalvm-runtime` — GraalVM Native Image bootstrap with `GraalVmInitializer.initialize()`, platform-specific reachability metadata, font substitution, and Skiko native library extraction. Requires BellSoft Liberica NIK 25 (full). Packaging via `packageGraalvmMacOS`, `packageGraalvmWindows`, `packageGraalvmLinux` tasks.
...
??? info "Sources"
    ...
    - **jDeploy**: [FAQ](https://www.jdeploy.com/docs/faq/) — "build native installers for Mac, Windows, and Linux on any platform"
    - **Potassium**: [`TargetFormat.kt`](https://github.com/sproctor/potassium/blob/main/plugin/src/main/kotlin/com/seanproctor/potassium/dsl/TargetFormat.kt) line 48 — `isCompatibleWithCurrentOS` check; packaging tasks disabled for non-matching OS
...
??? info "Sources"
    - **Potassium**: [`NsisSettings.kt`](https://github.com/sproctor/potassium/blob/main/plugin/src/main/kotlin/com/seanproctor/potassium/dsl/NsisSettings.kt) — 16 properties: `oneClick`, `allowElevation`, `perMachine`, `allowToChangeInstallationDirectory`, `createDesktopShortcut`, `createStartMenuShortcut`, `runAfterFinish`, `installerIcon`, `license`, `includeScript`, `multiLanguageInstaller`, `installerHeader`, `installerSidebar`, etc. [`DmgSettings.kt`](https://github.com/sproctor/potassium/blob/main/plugin/src/main/kotlin/com/seanproctor/potassium/dsl/DmgSettings.kt) — `background`, `backgroundColor`, `badgeIcon`, `icon`, `format` (6 DMG formats: UDRW, UDRO, UDCO, UDZO, UDBZ, ULFO), `window`, `contents`
...
??? info "Sources"
    - **Potassium**: [`TargetFormat.kt`](https://github.com/sproctor/potassium/blob/main/plugin/src/main/kotlin/com/seanproctor/potassium/dsl/TargetFormat.kt) — `isStoreFormat` = Pkg, AppX, Flatpak; [`PlatformSettings.kt`](https://github.com/sproctor/potassium/blob/main/plugin/src/main/kotlin/com/seanproctor/potassium/dsl/PlatformSettings.kt) — `appStore`, `entitlementsFile`, `provisioningProfile` for MAS; [`AppXSettings.kt`](https://github.com/sproctor/potassium/blob/main/plugin/src/main/kotlin/com/seanproctor/potassium/dsl/AppXSettings.kt) — `identityName`, `publisher`, `capabilities`; [`FlatpakSettings.kt`](https://github.com/sproctor/potassium/blob/main/plugin/src/main/kotlin/com/seanproctor/potassium/dsl/FlatpakSettings.kt) — `runtime`, `finishArgs`, `license`
...
The upstream project Potassium was forked from, now developed at [NucleusFramework/Nucleus](https://github.com/NucleusFramework/Nucleus). Nucleus positions itself as a full desktop application *framework*: the same electron-builder-based packaging core Potassium inherited (18 formats per its README, all three OSes on x64 + arm64, macOS universal binaries, GraalVM native-image and JDK 25+ AOT-cache support) plus 25+ runtime modules on Maven Central — native window decorations (Material 2/3, Jewel), notifications, dark-mode/accent detection, taskbar/dock and media-key integration, native SSL and HTTP clients. Its `nucleus.updater-runtime` auto-updates from GitHub/S3 with SHA-512 verification and progress tracking, but full-file downloads only — no blockmap-based delta updates. Plugin id `dev.nucleusframework`, MIT, very actively developed.

Because the packaging cores share lineage, most of the feature tables above read similarly for Nucleus. Potassium tracks a deliberately narrower scope — the packager and updater, nothing else — and has diverged where it counts for updates: blockmap-based differential downloads and a single-version release model. Choose Nucleus when you want the batteries-included framework rather than packaging alone.
...
| **Batteries-included framework** | Nucleus | 25+ runtime modules (windowing, notifications, media) | [GitHub](https://github.com/NucleusFramework/Nucleus) |
