...
Potassium injects the application's version and id into the packaged launcher (and the `run` task)
as JVM system properties, so your code can read them at runtime without bundling a generated
`BuildConfig` or reading a properties file:

| System property | Value | Source |
|-----------------|-------|--------|
| `app.version` | The application version | `packageVersion`, falling back to the Gradle project version. Omitted when neither is set. |
| `app.id` | The platform application id | macOS: `bundleID` → `macOS.packageName` → `packageName` → project name. Linux/Windows: per-OS `packageName` → `packageName` → project name. |

```kotlin
val version = System.getProperty("app.version")   // e.g. "1.2.3-beta.1"
val appId = System.getProperty("app.id")          // e.g. "com.example.myapp"
```

The [updater](auto-update.md) reads `app.version` automatically to detect the currently installed
version, and `app.id` to namespace its update marker.
